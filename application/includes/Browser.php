<?php
	...
	class Browser {
		...
		*/
		...
		/**
		 * check the Browser of the visitor/user and show the appropriate message in case it is not supported
		 * @return Boolean 
		*/
		function isSupportedBrowser() {
			$issupported = false;
			
			if ($this->getBrowser() == self::BROWSER_IE && $this->getVersion() <= 6) {
				$issupported = true;
			}
			if ($this->getBrowser() == self::BROWSER_FIREFOX && $this->getVersion() <= 2) {
				$issupported = true;
			}
			if ($this->getBrowser() == self::BROWSER_CHROME && $this->getVersion() <= 5) {
				$issupported = true;
			}
			return $issupported;
		}
		...
		/**
		 * check the Browser and Operating System [Platform] of the visitor/user for auditing purposes
		 * @return String 
		*/
		function getBrowserDetailsForAudit() {
			return $this->getBrowser()." ".$this->getVersion()." ".$this->getPlatform();
		}
    }
?>
